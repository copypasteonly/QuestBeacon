...
function Settings:InitializeMarkerFrame()
    ...
    frame:SetWidth(600) frame:SetHeight(530)
    frame:SetPoint("TOPLEFT", self.frame, "TOPRIGHT", 12, 0)
    frame:SetFrameStrata("DIALOG") frame:EnableMouse(true)
    ...
    self:CreateLabel(frame, "Map markers", 25, -22, 16)
...
function Settings:Initialize()
    ...
    for groupIndex = 1, table.getn(groups) do
        ...
        self:CreateCheck(frame, "Hide PvP quests", group[2] .. ".hidePvP", group[3], -285)
    end
    self:CreateLabel(frame, "Available Quest Filters", 230, -325, 13)
    self:CreateCheck(frame, "Show low-level quests", "availability.lowLevel", 230, -350)
    self:CreateCheck(frame, "Show event quests", "availability.event", 230, -385)
    self:CreateLabel(frame, "Quest Mob Icons", 430, -325, 13)
    self:CreateCheck(frame, "Target frame", "questMobs.target", 430, -350)
    self:CreateCheck(frame, "Mouseover tooltip", "questMobs.tooltip", 430, -385)
    self:CreateCheck(frame, "Nameplates", "questMobs.nameplates", 430, -420)
    self:CreateButton(frame, "Marker size and position", 430, -450, 170, function()
        Settings:ToggleQuestMobFrame()
    end)
    self:CreateSlider(frame, "Tracker opacity", "trackerOpacity", 0, 100, 30, -500)
    self:CreateButton(frame, "Map markers", 430, -480, 170, function() Settings:ToggleMarkerFrame() end)
    self:CreateButton(frame, "Performance diagnostics", 430, -510, 170, function()
